...
object DiscordWebhook {
        ...
        val discordEmbed =
            """{
                ...
                "embeds": [
                {
                    "color": 65535,
                    "author": {
                    "name": "$username has mined +$count Momentum",
                    "icon_url": "http://minotar.net/avatar/$uuid/64"
                }
                }
                ],
    ...
    fun sendArtReward(
        ...
        val newSource =
            when (source) {
                null, "" -> "from an event crate"
                "Mining" -> "Mined"
                "Fishing" -> "Fished"
                "Auto-Miner" -> "Auto-Mined"
                "OverDrive" -> "OverDrived"
                "OfflineAuto-Miner" -> "OfflineAuto-Mined"
                else -> ""
            }

        val message =
            if (source.isNullOrBlank()) {
                """$username found ${count}x $type Artifact from an Event Crate"""
            } else {
                """$username $newSource ${count}x $type Artifact"""
            }
